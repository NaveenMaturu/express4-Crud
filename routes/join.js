...
var User = sequelize.define('tab1s', {
...
var Post = sequelize.define('tab2s', {
"title": Sequelize.STRING,
...
//LEFT OUTER JOIN 'table1s' AS 'table1' ON 'table2'.'id' = 'table1'.'id' WHERE 'table2'.'title' = 'movie2';
/*router.get('/:title/:limit/:offset',function(req,res,next){
console.log("hi")
var cacheObj = cacher(sequelize,rc)
.model('table2')
.ttl(1000);
var lmt=req.param('limit');
var b = parseInt(lmt);

var off = req.param('offset');
var c = parseInt(off);
cacheObj.findAll({
where:{
title: req.params.title
},include:[User],limit:b,offset:c
}).then(function(jane){
res.json(jane);
}, function(error){
res.send(error);
...
{
User.findAndCountAll({}).then(function(user) {
var c= user.count
console.log(c);
...
rows.push({name: 'id' + JSON.stringify(data[i])});
